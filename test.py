...
from report_new_linter_errors import main, Sys, EXIT_CODE_NO_SNAPSHOT_FILE, \
    adjust_line_numbers, parse_git_diff_hunks, GitDiffHunk, DiffLine, \
    parse_snapshot_lines, SnapshotEntry
import example_linter

stubbed_git = 'stubbed-git.bat' if sys.platform == 'win32' else 'stubbed-git.py'
env_override = {
    'REPORT_NEW_LINTER_ERROR_PATH':
        os.path.join(os.path.dirname(__file__), 'test-tmp'),
    'REPORT_NEW_LINTER_ERROR_GIT_COMMAND':
        os.path.join(os.path.dirname(__file__), 'example', stubbed_git),
}
